...
const checkNoMoreThan1000Calories = async (value,{req}) => {
  try{
    const caloriesTotal = (req.body.fats * 9 )+( req.body.proteins *4 )+ (req.body.carbohydrates *4)
    if(caloriesTotal > 1000){
      return Promise.reject(new Error('The product has more than 1000 calories.'))
    }else{
...
const create = [
  ...
  check('calories').custom((value,{req})=> {
    return checkNoMoreThan1000Calories(value,{req})
  }),
  check('fats').isFloat().toFloat(),
  check('proteins').isFloat().toFloat(),
  check('carbohydrates').isFloat().toFloat(),
  check('calories').isFloat().toFloat()
]
